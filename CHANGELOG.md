...
- **REWRITTEN — `src/pages/en/glossaire/index.astro`** (~543 lignes, vs 237 lignes auparavant) : Glossaire EN entièrement enrichi avec **31 longDef + 31 example** traduits depuis le FR. Auparavant la page EN ne contenait que les `shortDef` (1 phrase chacun) avec un renvoi forcé au FR pour les détails. La nouvelle structure ajoute `slug` (partagé avec FR/AR pour cross-language linking), `longDef` (2-3 paragraphes par terme) et `example` (exemple business concret). Rendu transformé : chaque terme devient un `<details>` expandable HTML5 natif avec marker `+/−` qui se transforme à l'ouverture, séparation claire entre summary (term + shortDef) et details (longDef paragraphs + example aside). 31 termes traduits dans les 6 catégories : **Fundamentals** (5 — Artificial Intelligence, Narrow AI, General AI/AGI, Collective intelligence, Automation), **Machine Learning** (6 — ML, Deep Learning, Supervised, Unsupervised, Reinforcement, Training data), **Generative AI** (7 — Generative model, LLM, Foundation model, Hallucination, RAG, Prompt engineering, Fine-tuning), **Robotics** (3 — Robotics, Cobot, AMR), **Governance** (7 — Algorithmic bias, AI Governance, AI Risk Management, EU AI Act, Explainability, Accountability, Human-in-the-loop), **Methodology** (3 — AI roadmap, AI pilot, Capstone). Tous les concepts business critiques sont couverts avec exemples cités (Stripe Radar, Morgan Stanley, MIT Mirai, Amazon DeepFleet, Universal Robots, Takeda × MIT CCI Superminds, GitHub × Accenture, NIST AI RMF, EU AI Act 2024/1689, etc.). Page rendue : 80 KB en HTML (vs 18 KB avant).
- **REWRITTEN — `src/pages/ar/glossaire/index.astro`** (~542 lignes, vs 201 lignes auparavant) : Glossaire AR entièrement enrichi en parité avec EN — **31 longDef + 31 example** traduits depuis le FR vers l'arabe formel professionnel. Mêmes 31 entrées avec mêmes slugs (cross-language linking FR↔EN↔AR fonctionnel). Préservation systématique des termes anglais standards en latin (LLM, RAG, AGI, GPT-4, Claude, Llama, Mistral, NIST, AI Act, RGPD, ML, foundation model, fine-tuning, prompt engineering, human-in-the-loop, capstone, cobot, AMR, etc.) au sein du texte arabe — pratique professionnelle standard pour la documentation IA en arabe. Préservation également des noms propres en latin (Thomas Malone, Regina Barzilay, MIT CSAIL, MIT CCI, Stanford, Stripe, Morgan Stanley, Amazon, Takeda, Stanford HAI, World Economic Forum, etc.). Rendu identique au EN avec `<details>` expandable, mais en RTL (right-to-left) : utilisation systématique des CSS logical properties (border-inline-start, padding-inline, float: inline-end, scroll-margin-block-start) qui s'inversent automatiquement en RTL. Marker `+/−` apparaît correctement à gauche en mode RTL (inline-end devient l'opposé). Page rendue : 99 KB en HTML (vs 24 KB avant).
- Pattern d'enrichissement : interface `Term` étendue avec 3 nouveaux champs (`slug`, `longDef`, `example?`). longDef structuré avec `\n\n` pour séparer les paragraphes, splitté côté template et rendu en `<p>` séparés. Tous les termes ont longDef et example (aucun `example` manquant). Cross-language linking activé : `https://[domain]/fr/glossaire/#ia` et `https://[domain]/en/glossaire/#ia` et `https://[domain]/ar/glossaire/#ia` pointent désormais tous trois sur le même concept. CSS partagé entre EN et AR (~120 lignes), distincts du FR qui utilise un layout plus riche (avec ToC, navigation A-Z, cross-references entre termes via relatedTerms).
- Build validé : 98 pages, **0 erreur Astro check** sur **147 fichiers**. Sanity checks rendu HTML : EN — 31 items `<details>`, 31 occurrences "Business example", 6 catégories, slugs partagés (ia/machine-learning/llm/rag/fine-tuning/cobot/amr/ai-act/capstone/ia-faible) ; AR — 31 items `<details>`, 31 occurrences "مثال أعمال" + 2 du CSS = 33 total, slugs partagés identiques (10/10 vérifiés sur les 31), `dir="rtl"` correctement appliqué.

### Étape 4 — Capstone EN (terminée)

- **REWRITTEN — `src/pages/en/capstone/index.astro`** (~1 082 lignes, vs 316 lignes auparavant) : Capstone "Build your strategic AI roadmap" entièrement traduit en anglais avec parité structurelle complète au FR (1 088 lignes). Le capstone EN précédent était une version abrégée renvoyant au FR pour l'exemple annoté et les rubrics détaillés ; la nouvelle version contient l'intégralité du parcours pédagogique. **8 sections complètes** :
  1. **Capstone overview** — ExecutiveSummary 3 paragraphs (integrative final project, not a school deliverable, estimated duration 90-120 min canvas / 1-2 days complete / 2-3 weeks team workshop) + dl meta 4 rows (Audience executives + transformation leaders + business directors + consultants, Prerequisites min modules 1+5+6 ideally all 6, Expected deliverable 12-24 month roadmap one-page summary + annexes + self-assessment grid, Evaluation modality self-eval guided by 7×4 grid + optional peer review or simulated C-suite defense)
  2. **Learning objectives** — KeyTakeaways with 7 outcomes (vision specific not interchangeable, honest 8-dimension maturity diagnostic, contrasting scenarios + robust actions, V×F×R use case prioritization, 5-dimension 12-24 month plan, anticipate risks with verifiable mitigation, defend choices in front of demanding audience)
  3. **4-step methodology** — 4 detailed step articles in custom HTML (Step 1 Vision and maturity diagnostic 25-30 min with 8 maturity dimensions data/skills/governance/projects/compliance/change/infrastructure/social dialogue ; Step 2 Three contrasting scenarios 25-30 min with A Accelerated continuity / B Technological breakthrough / C Regulatory and social tightening + robust actions ; Step 3 Priority use cases V×F×R 25-30 min ; Step 4 12-24 month action plan 5 dimensions 30-45 min with sequenced vision / governance and compliance / skills and change management / infrastructure and data / metrics and KPIs)
  4. **Evaluation grid 7×4** — 7 detailed rubrics (Strategic vision / Maturity diagnostic / Use case selection and prioritization / Governance and compliance / Skills and change management / Operational plan budget and KPIs / Robustness to uncertainties) each with 4 explicit level descriptors (Insufficient / Emerging / Solid / Excellent) + aggregate criterion note for C-suite presentation (Solid on 5/7 + Excellent on 2/7)
  5. **Annotated example — Métallurgie de Précision SAS (fictional)** — Full pedagogical example: French SME precision metalworking ~150 employees 2 sites Auvergne+Alsace aerospace+medical, 3-year vision quote, maturity 14/40 with weak points (AI skills 1/5, AI governance 1/5, projects in production 1/5) and relative strengths (infrastructure 3/5 ERP+private cloud, compliance 3/5 already MDR), 3 contrasting scenarios A/B/C with robust actions, top-3 use cases (palletizing cobot V:5/F:4/R:4=13/15 €80k, RAG technical documentation V:4/F:3/R:4=11/15 €40k, supervised ML failure prediction V:4/F:2/R:4=10/15 €60k), 18-month plan with T0+6/T0+12/T0+18 milestones, €280-350k budget breakdown 50% technical / 30% HR / 15% external support / 5% governance with mixed financing self-financing + BPI France SME AI grant. 5 annotation blocks rating each section Solid to Excellent.
  6. **Executive checklist** — Full Checklist component with 18 points organized in 7 groups (Before starting / Step 1 Vision and maturity / Steps 2-3 Scenarios and use cases / Step 4 Action plan / Compliance and change management / Synthesis and validation / Defense and follow-up)
  7. **Download the canvas** — Markdown canvas link with CC BY-NC-SA 4.0 license note
  8. **Going further** — ResourceList with 3 linked resources (Module 6 / NIST AI RMF / EU AI Act 2024/1689) + CTA links to Module 6, resource hub, glossary, method
- Components 100% reused from FR version: BaseLayout, HeroSecondary, TableOfContents, ExecutiveSummary, KeyTakeaways, Checklist, ResourceList, SourceTag, PrintButton, IndependenceNotice, getLocalizedUrl. Same CSS (~270 lignes inline `<style>` block) — meta + steps + rubrics + example + download + cta + independence variants identical to FR.
- Build validé : 98 pages, **0 erreur Astro check** sur **147 fichiers**. Sanity checks rendu HTML : 8 sections (= FR), 7 rubric dimensions (= FR), 4 méthodologie steps (= FR), 5 blocs annotation example (= FR), Métallurgie de Précision SAS preserved as French name for authenticity.

### Étape 3 — Cas détaillés EN (terminée)

- **REWRITTEN — `src/pages/en/cas/mit-ide/index.astro`** (~517 lignes) : Cas MIT Initiative on the Digital Economy entièrement traduit en anglais (Module 6 main case — clôture du parcours). 11 sections complètes : Case identity (organization MIT IDE founded 2013 within MIT Sloan, scope research on impact of digital technologies on economy work organizations society, multi-disciplinary academic + practitioner translation approach, primary sources ide.mit.edu), Summary (one of the world's main academic devices for prospective research on future of work, structured strategic anticipation distinguished from forecasting and passive watch, transposable to organizational scale), Organizational context (5 structural characteristics : top-tier academic institution backing / multi-disciplinary approach / academic rigor + practitioner accessibility / continuity over time / links with other MIT initiatives CSAIL CCI Jameel Clinic), Problem addressed (5 problems organizations encounter when anticipating : going beyond media announcements / articulating technical-economic-social dimensions / producing transposable frameworks / anchoring in empirical data / building durability), Methodological approach (6 sub-sections describing : Rigorous academic research / Multi-disciplinary approach / Empirical data and case studies / Translation for practitioners / Capitalization over time / Articulation with other initiatives), Observed contributions (5 items : public debate framing / methodological recommendations applied to GitHub × Accenture / influence on public policies OECD World Bank / pedagogical resources for practitioners / inspiring institutional model for Stanford HAI Oxford FHI), Limits and challenges (5 items : academic scale ≠ organizational scale / slow academic pace / objectivity never total / transposition requires adaptation work / risk of academic over-investment), Usage risks (4 items : citing without applying / over-confidence in conclusions / delay in operational translation / analytical paralysis), Managerial takeaways (5 items emphasizing strategic anticipation as investment), Pedagogical discussion questions (5 items applied to organization context), Modules and sources (Module 6 main case + Module 1/5 references + ResourceList with MIT IDE official + MIT CCI generative AI + WEF Future of Jobs).
- **REWRITTEN — `src/pages/en/cas/ai-act-eu-haut-risque/index.astro`** (~537 lignes) : Cas EU AI Act — Classification haut risque entièrement traduit en anglais (Module 5 mini-case). 11 sections complètes : Case identity (legal reference Regulation EU 2024/1689 adopted May 2024 published OJEU July 12 2024 in force August 1 2024, progressive application until August 2027, risk-based approach 4 categories, sanctions up to 35M€ or 7% turnover GDPR-comparable, primary sources EUR-Lex + European Commission), Summary (world's first global structured AI regulatory framework, 4 categories prohibited/high-risk/limited risk/minimal risk, many ordinary business uses fall under high-risk, classification is the first step), Drafting context (5 structural characteristics : world's first global framework / risk-based regulatory innovation / broad scope of application with extraterritorial effect / articulation with GDPR + sectoral regulations + ISO 42001 / progressive application until 2027), Problem addressed (5 structural problems : protect fundamental rights / avoid most dangerous uses / harmonize single market / articulate with European digital sovereignty / create durable framework), AI Act architecture (6 sub-sections : Prohibited practices Article 5 / High-risk systems Annex III with 8 sub-categories from infrastructure to medical devices and Articles 8-17 obligations / Limited-risk systems Article 50 transparency / Minimal risk / General-purpose models Articles 51-56 with systemic risk threshold / Application calendar 2025-2027), Observed benefits (5 items : international reference effect UK US Brazil Korea Japan / structuring of internal governance / emergence of compliance ecosystem / protection of persons / legal clarity for economic actors), Limits and challenges (6 items : technical complexity / compliance cost potentially substantial / general-purpose models evaluation under construction / overlap with MDR + GDPR / dependence on national authorities + AI Office / pace of technological evolution outpacing legal framework), Implementation risks (5 items : misclassification / formal compliance without real application / innovation delay through over-caution / documentary over-investment / regulatory evolution), Managerial takeaways (5 items), Pedagogical discussion questions (5 items applied to organization context with NIST AI RMF articulation), Modules and sources (Module 5 mini-case complementary to NIST AI RMF main case + Module 6 reference + ResourceList with EUR-Lex EN + European Commission DG CNECT + NIST AI RMF).
- Build validé : 98 pages, **0 erreur Astro check** sur **147 fichiers**. Contenu EN rendu correctement (AI Act 15x, risk categories 18x ; MIT IDE 26x, anticipation/scenario 18x).


- **REWRITTEN — `src/pages/en/cas/nist-ai-rmf/index.astro`** (~516 lignes) : Cas NIST AI Risk Management Framework entièrement traduit en anglais (Module 5 main case). 11 sections complètes : Case identity (organization NIST National Institute of Standards and Technology, framework AI RMF 1.0 published January 2023, voluntary and non-binding, primary sources nist.gov + AI 100-1 PDF), Summary (most internationally recognized operational AI governance framework, 4 functions Govern/Map/Measure/Manage forming a system not a checklist, complementary to EU AI Act not competing, principles without a system are ornamental), Context (4 NIST characteristics : standards body trusted in industry / methodological rigor + iterative public consultation / scope explicitly trustworthy AI multi-criteria / mandate to make this kind of framework), Problem addressed (organizations have AI principles but no operational method to translate them into processes, roles, measures), Framework structure (5 sub-sections describing the 4 functions Govern transverse / Map context / Measure risks / Manage treatment + iterative cyclical articulation), Observed benefits (5 items : adopted internationally / international compatibility / orientation framework not legal text / coherence with NIST cybersecurity framework / methodological credibility), Limits and challenges (5 items : voluntary not mandatory / generic by design needs adaptation / requires governance maturity / does not replace AI Act for high-risk systems / requires sustained investment), Usage risks (5 items : ornamental adoption without depth / confusion with AI Act / paralysis seeking exhaustivity / underestimating Govern function / risk of disconnect from operational reality), Managerial takeaways (5 items), Pedagogical discussion questions (5 items), Modules and sources (Module 5 main case + Module 6 reference + cross-link to ai-act-eu-haut-risque + ResourceList with NIST official + AI RMF 1.0 PDF + OECD AI Principles).
- **REWRITTEN — `src/pages/en/cas/universal-robots/index.astro`** (~508 lignes) : Cas Universal Robots — Cobots pour PME industrielles entièrement traduit en anglais (Module 4 mini-case). 11 sections complètes : Case identity (organization Universal Robots A/S Danish Teradyne since 2015, technology cobots ISO/TS 15066 compliant, product range UR3e/UR5e/UR10e/UR16e/UR20/UR30, UR+ ecosystem of integration partners, primary sources universal-robots.com), Summary (collaborative robotics not reserved for large industries, modular design + ease of programming + safety standards open robotics to industrial SMEs, ecosystem more than the cobot itself makes the solution viable), Context (4 SME industrial constraints : limited size and capital / heterogeneous low-volume production / few internal automation skills / requires fast time-to-value), Business problem (3 historical barriers to industrial automation for SMEs : prohibitive cost / fixed-task rigidity / safety cage incompatible with shared environment), AI solution implemented (5 sub-sections : Cobot designed for collaboration / ISO/TS 15066 compliance and 4 collaboration modes / Programming accessible to non-experts / UR+ ecosystem of certified accessories / Specialized integrators network), Observed benefits (5 items : access to industrial automation for SMEs / shorter return on investment / reconfigurability for varied production / co-located rather than caged operation / structuring effect on the cobot category), Limits and challenges (5 items : the cobot is only part of the cost / use cases more bounded than industrial robots / requires structured integration even simplified / risk assessment remains mandatory / regulations evolving with EU AI Act and EU Machinery), Identified risks and guardrails (5 items : underestimating risk assessment / payload and reach overestimation / cybersecurity of connected cobot / progressive integrator dependency / EU AI Act + EU Machinery), Managerial takeaways (5 items), Pedagogical discussion questions (5 items), Modules and sources (Module 4 mini-case + cross-link to amazon-robotics + ResourceList with Universal Robots corporate + ISO/TS 15066 + MIT OpenCourseWare 2.12).
- Build validé : 98 pages, **0 erreur Astro check** sur **147 fichiers**. Contenu EN rendu correctement (NIST 37x, Govern/Map/Measure/Manage 26x ; Universal Robots/cobot 48x, ISO 10218/15066 14x).
- **REWRITTEN — `src/pages/en/cas/github-copilot-accenture/index.astro`** (~510 lignes) : Cas GitHub Copilot × Accenture entièrement traduit en anglais (Module 3 mini-case). 11 sections complètes : Case identity (organizations GitHub + Accenture ~700 000 employés, technology Copilot, methodology treatment vs control), Summary (rigorous measurement methodology, transposable beyond code), Organizational context (4 Accenture characteristics : scale / diversity / process and data maturity / direct business stake), Business problem (3 facets : quantify real impact / measure several dimensions / isolate tool's own effect), Methodological solution implemented (5 sub-sections : Treatment vs control group / Multiple metrics / Sufficient duration / Integrated change management / Transparent publication), Observed benefits (5 items : productivity / code quality no regression / developer satisfaction / retention effect / methodological credibility), Limits and challenges (5 items : context-specific / GitHub conflict of interest / open junior learning question / unmeasured effects / emerging regulatory compliance), Identified risks and guardrails (5 items : intellectual property / client data leak / generated code quality / long-term learning / EU AI Act), Managerial takeaways (5 items), Pedagogical discussion questions (5 items), Modules and sources (Module 3 mini-case + Module 5/6 references + ResourceList).
- **REWRITTEN — `src/pages/en/cas/mit-barzilay-cancer/index.astro`** (~510 lignes) : Cas Recherche MIT — Regina Barzilay (cancer screening) entièrement traduit en anglais (Module 2 mini-case). 11 sections complètes : Case identity (organization MIT CSAIL + Jameel Clinic, principal researcher Barzilay, technology Mirai deep learning, primary sources csail.mit.edu + MIT News), Summary (Mirai breast cancer 1-5 years prediction, methodological standard for high-stakes AI), Organizational context (4 structural characteristics : access to labeled medical data / academic methodological rigor / interdisciplinary collaboration / Barzilay personal commitment), Business problem (4 limits of traditional risk scores : Tyrer-Cuzick BRCAPRO Gail / mammography contains more information / population diversity / consequences of error), AI solution implemented (5 sub-sections : Supervised deep learning on images / Labeling by qualified experts / Independent multi-site validation / Non-negotiable human supervision / Transparency on limits), Observed benefits (5 items : superior predictive performance / robustness to population diversity / personalized screening contribution / methodological credibility / structuring pedagogical effect), Limits and challenges (5 items : labeling limiting factor / generalization to confirm / slow clinical adoption / GDPR and EU AI Act / radiologist profession evolution), Identified risks and guardrails (5 items : ethnic bias / post-deployment degradation / ethical risk / false confidence / EU AI Act high-risk MDR), Managerial takeaways (5 items), Pedagogical discussion questions (5 items), Modules and sources (Module 2 mini-case + Module 5 central + Module 1 reference + ResourceList).
- **REWRITTEN — `src/pages/en/cas/amazon-robotics/index.astro`** (~505 lignes) : Cas Amazon Robotics — Un million de robots et DeepFleet entièrement traduit en anglais. 11 sections complètes : Case identity (organization Amazon + Kiva 2012, technology AMR + DeepFleet foundation model, scale 1M+ robots, primary sources About Amazon Operations), Summary (one million robots threshold, DeepFleet as language-LLM analogue, three pillars + place of human + competitive advantage), Organizational context (4 characteristics of warehouse logistics : volumes / partially structured environment / repetitive but varied tasks / human-machine safety), Business problem (4 questions emerging at scale : heterogeneous fleet orchestration / network-scale learning sharing / multi-objective optimization / human integration), AI solution implemented (5 sub-sections : Specialized multi-generation fleet / DeepFleet foundation model / Shared learning at network scale / Augmenting humans rather than replacing / Human-machine safety), Observed benefits (5 items : processing capacity / operational precision / new roles / structural competitive advantage / reusable platform), Limits and challenges (5 items : massive investment / lock-in effect / working conditions / software dependency / EU AI Act regulations), Identified risks and guardrails (4 items : physical safety / social and change management / cyber / drift toward excessive autonomy), Managerial takeaways (5 items), Pedagogical discussion questions (5 items), Modules and sources (Module 4 main + Module 5/6 references + ResourceList).
- **REWRITTEN — `src/pages/en/cas/stripe-radar/index.astro`** (~480 lignes) : Cas Stripe Radar — Détection de fraude par ML entièrement traduit en anglais. 11 sections complètes : Case identity (organization Stripe online payments, technology supervised learning binary classification + business rules + user feedback, volume hundreds of millions of labeled transactions, primary sources Stripe official guide), Summary (real-time scoring under 100 ms, multi-layer system, scale + orchestration as performance conditions, structural network effect), Organizational context (4 structural fraud challenges : organized adaptive fraudsters / merchant losses chargebacks / costly false positives / real-time constraint), Business problem (3 facets : maximize detection / minimize false positives / allow merchant customization), AI solution implemented (4 sub-sections : Supervised ML model trained at scale / User feedback integrated into training cycle / Customizable business rules / Real-time scoring and explainability), Observed benefits (4 items : significant chargeback reduction / rapid adaptation / positive network effect / customization without ML expertise), Limits and challenges (4 items : probabilistic tool / network effect benefits established players / not transposable to all domains / growing regulatory dimension), Identified risks and guardrails (4 items : discriminatory bias / arms race / merchant dependency / EU AI Act high-risk), Managerial takeaways (5 items), Pedagogical discussion questions (5 items), Modules and sources (Module 2 main + Module 5 reference + ResourceList).
- **REWRITTEN — `src/pages/en/cas/morgan-stanley/index.astro`** (~530 lignes) : Cas Morgan Stanley AskResearchGPT et Debrief entièrement traduit en anglais. 11 sections complètes : Case identity (organization, sector, technology, deployment scope avec les deux assistants AI @ Morgan Stanley Assistant + Debrief, primary sources), Summary (~16 000 conseillers, valeur dans l'orchestration pas le modèle, 70-80% écosystème vs 20-30% LLM), Organizational context (4 caractéristiques structurelles du métier Wealth Management), Business problem (deux facettes Assistant + Debrief avec workflow decomposition), AI solution implemented (5 sub-sections : Foundation model licensed not developed / RAG on curated internal corpus / Curation and structuring / Human escalation and scope limits / Governance and change management), Observed benefits (5 items), Limits and challenges (5 items), Identified risks and guardrails (4 items SEC/FINRA/data leak/bias/profession degradation), Managerial takeaways (5 items), Pedagogical discussion questions (5 items), Modules and sources (Module 3 main case + Module 1 mini-case + Module 5/6 references + ResourceList).
- **REWRITTEN — `src/pages/en/cas/takeda-superminds/index.astro`** (~610 lignes) : Cas Takeda × MIT CCI Superminds entièrement traduit en anglais. 11 sections complètes : Case identity (organization Takeda + MIT CCI partnership, conceptual framework Thomas Malone superminds, primary sources cci.mit.edu), Summary (mixed human × machine teams, collective intelligence framework, strategic question shift), Organizational context (4 structural challenges of pharma R&D + MIT CCI), Business problem (3 structural questions about creative phases / heterogeneous data integration / multi-actor collaboration), AI solution implemented (4 sub-sections : Mixed teams by construction / Explicit role orchestration / Analytical and AI tools as support / Structured collective learning), Observed benefits (4 items focused on organizational and strategic dimensions), Limits and challenges (4 items skill-demanding / collaboration culture prerequisite / indirect benefits / fashion effect risk), Identified risks and guardrails (3 items drift toward simple automation / scientific risk / organizational and social risk), Managerial takeaways (5 items), Pedagogical discussion questions (5 items), Modules and sources (Module 1 main + Module 5 reference + ResourceList).
- Build validé : 98 pages, **0 erreur Astro check** sur **147 fichiers**. Contenu EN rendu correctement (Takeda 16x, supermind 19x ; Morgan Stanley 19x, GPT-4 11x, RAG 11x ; Stripe 22x, fraud 24x ; Amazon 25x, DeepFleet 14x, ISO 4x ; Barzilay 12x, Mirai 8x ; Copilot 15x, Accenture 13x).
- Pattern préservé : 11 sections fixes, mêmes composants (BaseLayout, HeroSecondary, TableOfContents, ExecutiveSummary, KeyTakeaways, ResourceList, SourceTag, PrintButton, IndependenceNotice, PrevNextNav), CSS compactée identique au FR.

### Étape 2 — Modules EN (terminée)

- **NEW — `src/content/quizzes/module-6-en.ts`** (~145 lignes) : version anglaise du quiz Module 6 (The Future of AI). Mêmes 7 questions, mêmes options, mêmes correctIndex, mêmes scoring bands que le quiz FR — seul le contenu naturel est traduit.
- **REWRITTEN — `src/pages/en/modules/06-futur-ia/index.astro`** (~880 lignes) : Module 6 entièrement traduit en anglais — module de clôture du parcours pédagogique. 11 sections complètes : Executive summary (uncertainty justifies scenario planning, watch vs anticipation, 5-dimension roadmap), Learning objectives, Key concepts (5 ConceptCards : Scenario planning vs forecasting [Royal Dutch Shell 1970s], Expected technical evolutions at 3-10 years [multi-modality, autonomous agents, frugal AI, sectoral specialization, tools and platforms], Building a strategic AI roadmap [5 interdependent dimensions], Technology watch and strategic anticipation [4 components: dedicated cell, regular cycle, formalized deliverables, articulation with decision], Temporal hierarchization of the roadmap [0-6 / 6-12 / 12-24 months]), Common errors (4 PitfallList items : predicting single future, purely technological roadmap, confusing watch and anticipation, underestimating announcement-to-adoption delay), Real case MIT Initiative on the Digital Economy + mini-cas Future of Jobs Report (WEF) avec distinction reskilling/upskilling, Practical activity (4 steps : AI vision and maturity mapping / Build 3 contrasting scenarios / Translate into priority use cases / 12-24 month action plan), Formative quiz (interactive React island avec quizModule6EN), Manager checklist (12 points en 3 groupes : Vision and anticipation / Structured roadmap / Budget risks and execution), Application in your organization (5 questions + aside consultants pour préparation capstone), Key takeaways (avec conclusion du parcours et lien explicite au capstone), Going further (6 ressources). Build validé : 98 pages, 0 erreur Astro check sur **147 fichiers** (+1 vs précédent = quiz module-6-en.ts ajouté). Contenu EN rendu correctement (scenario planning 9x, MIT IDE 5x, World Economic Forum/Future of Jobs 4x, 5 dimensions 9x, capstone 12x).
- **NEW — `src/content/quizzes/module-5-en.ts`** (~145 lignes) : version anglaise du quiz Module 5.
- **REWRITTEN — `src/pages/en/modules/05-ia-societe/index.astro`** (~880 lignes) : Module 5 entièrement traduit en anglais (governance, ethics, EU AI Act, GDPR, David Autor, NIST AI RMF). 11 sections complètes avec 5 ConceptCards, 4 PitfallList items, real case NIST AI RMF + mini-cas EU AI Act high-risk classification, 12-checkpoint manager checklist en 3 groupes.
- **NEW — `src/content/quizzes/module-4-en.ts`** (~145 lignes) : version anglaise du quiz Module 4.
- **REWRITTEN — `src/pages/en/modules/04-robotique/index.astro`** (~850 lignes) : Module 4 entièrement traduit en anglais (industrial / cobot / AMR, ISO 10218, ISO/TS 15066, real total cost 50-70% integration). 11 sections complètes avec 5 ConceptCards, 4 PitfallList items, real case Amazon Robotics + DeepFleet + mini-cas Universal Robots SME robotics.
- **NEW — `src/content/quizzes/module-3-en.ts`** (~145 lignes) : version anglaise du quiz Module 3.
- **REWRITTEN — `src/pages/en/modules/03-ia-generative/index.astro`** (~820 lignes) : Module 3 entièrement traduit en anglais (foundation models, prompting/RAG/fine-tuning, hallucinations, total cost 6 lines). 11 sections complètes avec 5 ConceptCards, 4 PitfallList items, real case Morgan Stanley AskResearchGPT/Debrief + mini-cas GitHub Copilot × Accenture.
- **NEW — `src/content/quizzes/module-2-en.ts`** (~150 lignes) : version anglaise du quiz Module 2.
- **REWRITTEN — `src/pages/en/modules/02-machine-learning/index.astro`** (~810 lignes) : Module 2 entièrement traduit en anglais (3 ML families, 4 viability conditions, overfitting, model = 1 layer of system, strategic framing). 11 sections complètes avec 5 ConceptCards, 4 PitfallList items, real case Stripe Radar + mini-cas Regina Barzilay (MIT CSAIL).
- **NEW — `src/content/quizzes/module-1-en.ts`** (~145 lignes) : version anglaise du quiz Module 1. Pattern réutilisable pour les modules suivants.
- **REWRITTEN — `src/pages/en/modules/01-introduction-ia/index.astro`** (~800 lignes) : Module 1 entièrement traduit en anglais (narrow vs general AI, 3 history waves, collective intelligence with Thomas Malone, AI as socio-technical system, strategic framing). 11 sections complètes avec 5 ConceptCards, 4 PitfallList items, real case Takeda × MIT CCI + mini-cas Morgan Stanley. Composants 100% réutilisés (TableOfContents, ExecutiveSummary, ConceptCard, PitfallList, CaseCardCompact, Activity, QuizInteractive, Checklist, KeyTakeaways, ResourceList, PrevNextNav, PrintButton, IndependenceNotice).

**Bilan Étape 2 (Modules EN) — 100% complète** : 6/6 modules traduits, ~5 100 lignes EN ajoutées (~4 200 lignes pages + ~870 lignes quiz). 7 commits successifs (`0fe5206`, `4845d1b`, `a23d12c`, `f194529`, `c6805bd`, et le commit final Module 6). Build validé : 98 pages, **0 erreur / 0 warning / 0 hint** Astro check sur **147 fichiers**. Pattern : structure FR strictement préservée. CSS, components, layout identiques. Seul le contenu naturel (texte, titres, descriptions, h1/h2/h3, items des listes, descriptions des cards) est traduit. Les imports composants pointent vers les mêmes composants partagés. Conséquence : si on modifie un composant, les versions FR et EN bénéficient automatiquement.
...
- Phase 7 sub-2 — Pages-cadre EN complètes + pages d'agrégation EN structurées :
  ...
  - **REWRITTEN — `src/pages/en/a-propos/index.astro`** (~165 lignes) : page À propos complète. 6 sections : auteur, choix de MIT Sloan comme référence, justification du trilinguisme, philosophie en 3 principes, ouverture du projet, contact via GitHub Issues.
  - **REWRITTEN — `src/pages/en/mentions-legales/index.astro`** (~195 lignes) : Legal notice complète conforme RGPD. 8 sections : éditeur, hébergement OVH, propriété intellectuelle (code MIT + contenu CC BY-NC-SA), marques tierces (fair use nominatif), données personnelles (zéro collecte par défaut), localStorage quiz, logs serveur, droit applicable français.
  - **REWRITTEN — `src/pages/en/modules/index.astro`** (~90 lignes) : index des 6 modules avec `ModuleCard`s, message « publication status » expliquant que les modules sont complets en FR et progressivement traduits en EN.
  - **REWRITTEN — `src/pages/en/cas/index.astro`** (~218 lignes) : index des 10 études de cas avec `CaseCardCompact` pour chacun, organisation par module (M1-M6), tagline traduite pour les 10 cas, note explicative que les pages détaillées sont en FR pour le moment.
  - **REWRITTEN — `src/pages/en/capstone/index.astro`** (~270 lignes) : version structurée de la page capstone. Méthodologie en 4 étapes détaillées avec durée typique, grille d'évaluation 7×4 visuelle (dots colorés), checklist de pré-soumission en 18 points, lien explicite vers la version FR pour l'exemple pédagogique de la PME fictive Métallurgie de Précision SAS.
  - **REWRITTEN — `src/pages/en/faq/index.astro`** (~210 lignes) : 10 questions clés organisées en 4 sections thématiques (about / method / strategy / governance). Réponses HTML structurées avec liens internes vers méthode + modules. Note explicite que la version FR contient les 20 questions complètes (avec questions sur capstone, modules, multilangue).
  - **REWRITTEN — `src/pages/en/glossaire/index.astro`** (~330 lignes) : index alphabétique des **31 termes** avec leur version EN + version FR de référence + définition courte EN (2-3 phrases) pour chaque terme. Organisation par 6 catégories (fundamentals / ml / genai / robotics / governance / methodology). Lien vers la version FR pour les définitions détaillées avec exemples et cross-références.
  - **REWRITTEN — `src/pages/en/ressources/index.astro`** (~190 lignes) : hub des 31 ressources organisées en 8 familles (official-course / mit-sloan-article / mit-executive-article / mit-ocw / academic-paper / standard / case-source / external-resource). Labels et descriptions de catégories en EN, titres et auteurs des ressources préservés (la majorité sont en EN par nature).
  - **REWRITTEN — `src/pages/en/podcasts/index.astro`** (~470 lignes) : index podcasts EN aligné au pattern FR. Hero stats, intro avec note de transparence sur le caractère trilingue (épisodes en FR d'abord, EN plus tard), banner de progression auto-calculé avec compteurs accordés singulier/pluriel, 4 cartes thèmes avec labels et descriptions EN, sections par thème avec PodcastCards, série complète dans l'ordre.
  - **REWRITTEN — `src/pages/en/podcasts/production.astro`** (~310 lignes) : page de transparence éditoriale EN. 6 points de transparence, workflow en 5 étapes vulgarisé, grille qualité 8 critères, procédure signalement erreur via GitHub, limites assumées du dispositif.
  - **MINOR — `src/i18n/translations.ts`** : enrichissement des chaînes `translation-pending.lead` dans les 3 langues. Avant : « This page is available in French at this stage. The English translation is scheduled for Phase 7. ». Après : « This page is complete in French. Its English translation is in progress as part of Phase 7 of the project. You can read the full French version right now using the link below. » — message plus actionnable pour les pages encore en stub.
  - Build validé : 98 pages, **0 erreur / 0 warning / 0 hint** Astro check sur 141 fichiers. Toutes les 11 pages EN refondues rendues correctement (vérification HTML inline).
  - Pattern : 0 nouveau composant. Réutilisation totale de l'existant. Stratégie « contenu central traduit + pages denses condensées avec lien vers FR ». Décision pragmatique pour livrer un site EN cohérent et navigable sans bloquer pendant 8h+ de traduction de Module 1-6 (~5 000 lignes), Capstone détaillé (1088 lignes), Glossaire détaillé (697 lignes), FAQ complète (592 lignes), Ressources détaillées (725 lignes). Sub-3 finalisera les traductions AR avec validation RTL.
