...
- **NEW — `src/content/quizzes/module-5-en.ts`** (~145 lignes) : version anglaise du quiz Module 5 (AI in Business and Society). Mêmes 7 questions, mêmes options, mêmes correctIndex, mêmes scoring bands que le quiz FR — seul le contenu naturel est traduit.
- **REWRITTEN — `src/pages/en/modules/05-ia-societe/index.astro`** (~880 lignes) : Module 5 entièrement traduit en anglais. 11 sections complètes : Executive summary (governance is structuring not optional, ethics is more demanding than compliance, three operational components needed), Learning objectives, Key concepts (5 ConceptCards : Ethics and compliance - two distinct levels, Operational AI governance, The EU AI Act and use classification, AI impact on work - the Autor perspective, Social dialogue GDPR and extended compliance), Common errors (4 PitfallList items : confusing ethics and compliance, launching ethics committee without decision power, underestimating EU AI Act and GDPR, AI washing without real mechanisms), Real case NIST AI RMF + mini-cas EU AI Act high-risk classification, Practical activity (4 steps : define scope and principles / structure operational governance / map legal obligations / define transparency explainability and redress), Formative quiz (interactive React island avec quizModule5EN), Manager checklist (12 points en 3 groupes : Governance and AI committee / Ethics bias and impact / Compliance and transparency), Application in your organization (5 questions + aside consultants), Key takeaways, Going further (6 ressources). Build validé : 98 pages, 0 erreur Astro check sur 146 fichiers (+1 vs précédent = quiz module-5-en.ts ajouté). Contenu EN rendu correctement (AI Act 20x, NIST 9x, David Autor 6x, GDPR 15x).
- **NEW — `src/content/quizzes/module-4-en.ts`** (~145 lignes) : version anglaise du quiz Module 4 (Robotics in Business). Mêmes 7 questions, mêmes options, mêmes correctIndex, mêmes scoring bands que le quiz FR — seul le contenu naturel est traduit.
- **REWRITTEN — `src/pages/en/modules/04-robotique/index.astro`** (~850 lignes) : Module 4 entièrement traduit en anglais. 11 sections complètes : Executive summary, Learning objectives, Key concepts (5 ConceptCards : Three robotic families for three use cases, Perception planning and control - the three pillars, Human-machine safety and normative framework, The real total cost of a robotics project, Economic and social framing), Common errors (4 PitfallList items), Real case Amazon Robotics + DeepFleet + mini-cas Universal Robots et la robotique pour PME, Practical activity (4 steps), Formative quiz (interactive React island avec quizModule4EN), Manager checklist (12 points en 3 groupes), Application in your organization (5 questions + aside consultants), Key takeaways, Going further (6 ressources).
- **NEW — `src/content/quizzes/module-3-en.ts`** (~145 lignes) : version anglaise du quiz Module 3 (Generative AI). Mêmes 7 questions, mêmes options, mêmes correctIndex, mêmes scoring bands que le quiz FR — seul le contenu naturel est traduit.
- **REWRITTEN — `src/pages/en/modules/03-ia-generative/index.astro`** (~820 lignes) : Module 3 entièrement traduit en anglais. 11 sections complètes : Executive summary, Learning objectives, Key concepts (5 ConceptCards : Foundation models and economics of GenAI, Prompting RAG and fine-tuning, Hallucinations and guardrails, Decompose the workflow rather than automate, The total cost of automation), Common errors (4 PitfallList items), Real case Morgan Stanley AskResearchGPT/Debrief + mini-cas GitHub Copilot × Accenture, Practical activity (4 steps), Formative quiz (interactive React island avec quizModule3EN), Manager checklist (12 points en 3 groupes), Application in your organization (5 questions + aside consultants), Key takeaways, Going further (6 ressources).
- **NEW — `src/content/quizzes/module-2-en.ts`** (~150 lignes) : version anglaise du quiz Module 2. Mêmes 7 questions, mêmes options, mêmes correctIndex, mêmes scoring bands que le quiz FR — seul le contenu naturel est traduit.
- **REWRITTEN — `src/pages/en/modules/02-machine-learning/index.astro`** (~810 lignes) : Module 2 entièrement traduit en anglais. 11 sections complètes : Executive summary, Learning objectives, Key concepts (5 ConceptCards : The three ML families, The four viability conditions, Overfitting and generalization, The model is just one layer of the system, Strategic framing of an ML use case), Common errors (4 PitfallList items), Real case Stripe Radar + mini-cas Mirai (Regina Barzilay MIT CSAIL), Practical activity (4 steps), Formative quiz (interactive React island avec quizModule2EN), Manager checklist (12 points en 3 groupes), Application in your organization (5 questions + aside consultants), Key takeaways, Going further (6 ressources).
